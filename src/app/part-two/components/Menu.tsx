...
import {
  ...
  handleOverflow,
  handleHeight,
} from "@/app/part-two/lib/mega-menu";
...
>(function MenuComponent({ page, children, ...props }, ref) {
  ...
  const { floatingStyles, refs, context } = useFloating<HTMLButtonElement>({
    ...
    middleware: [shift(), handleOverflow(), handleHeight(nodeId, setHeight)],
  ...
  /**
   * Unset max height variable if l1 is closed
   */
  useEffect(() => {
    if (isNested || isOpen) return;

    setHeight([]);
    document.documentElement.style.setProperty("--menu-max-height", "unset");
  }, [isNested, isOpen, setHeight]);

  /**
   * Set max height variable when max height changes
   */
  useEffect(() => {
    if (height.length === 0) return;

    const [, px] = height[height.length - 1];
    document.documentElement.style.setProperty("--menu-max-height", `${px}px`);
  }, [height]);
  ...
  useEffect(() => {
    if (isNested && !isOpen) {
      const ref = tree?.nodesRef.current.find((ref) => ref.id === nodeId);

      const toRemove = height.find(([id]) => id === ref?.id);

      if (toRemove) {
        const newHeights = height.filter((i) => i !== toRemove);
        setHeight(newHeights);
      }
    }
  }, [height, isNested, isOpen, nodeId, setHeight, tree?.nodesRef]);
